                ...
                0 !== l.length && (r || (l[d.config.indexName] = a), c.eachCols(function (n, r) {
                    ...
                    if (void 0 !== v && null !== v || (v = ""), !r.colGroup) {
                        var m = ['<td data-field="' + c + '" data-key="' + h + '" ' + function () {
                            ...
                            /** 插入日期的格式化 */
                            if(r.field == "date"){
                                var temp_ = (String(v).indexOf("-") != -1)?v:[String(v).substring(0, 4), String(v).substring(4, 6), String(v).substring(6, 8)].join('-');
                                if(new Date(temp_).getDay() == 6){
                                    return "<span class='layui-badge layui-bg-green' style='padding: 0px 3px;margin-left: 10px'>六</span>"
                                }if(new Date(temp_).getDay() == 0){
                                    return "<span class='layui-badge' style='padding: 0px 3px;;margin-left: 10px'>日</span>"
                                }else{
                                    return ""
                                }
                            }else{
